...
private:
	//reference to graph to be searched
...
public:
	Graph_SearchDFS(const graph_type& graph, int source, int target = -1 )
		: m_Graph(graph), m_iSource(source), m_iTarget(target), m_bFound(false), 
		m_Visited(m_Graph.NumNodes(), unvisited), m_Route(m_Graph.NumNodes(), no_parent_assigned)
	{
		m_bFound = Search();
	}
...
//----------------------------- Graph_SearchBFS -------------------------------
//	class to implement a breadth first search. 
//-----------------------------------------------------------------------------
template<class graph_type>
...
public:
	Graph_SearchBFS(const graph_type& graph, int source, int target = -1 )
		...
		m_bFound(false), m_Visited(m_Graph.NumNodes(), unvisited),
